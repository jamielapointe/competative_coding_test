...
static void BM_01d_bad_hint(benchmark::State& state) {
  srand(1);
  ...
  for (auto _ : state) {
    ...
    int64_t a2 = 0;
    for (size_t i = 0; i < N; ++i) {
      if (__builtin_expect(b1[i], false)) {  // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
...
BENCHMARK(BM_01d_bad_hint)->Arg(1 << SHIFT_FACTOR);  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables,cppcoreguidelines-owning-memory,clang-diagnostic-global-constructors)
// clang-format on
