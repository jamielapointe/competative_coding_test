...
static void BM_01b_branch_mispredicted(benchmark::State& state) {
  srand(1);
  ...
  for (size_t i = 0; i < N; ++i) {
    v1[i] = rand();
    v2[i] = rand();
    c1[i] = rand() & 0x1;  // pseudo-randomly true/false
  }
  auto* p1 = v1.data();
  auto* p2 = v2.data();
  int* b1 = c1.data();
...
BENCHMARK(BM_01b_branch_mispredicted)->Arg(1 << SHIFT_FACTOR);  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables,cppcoreguidelines-owning-memory,clang-diagnostic-global-constructors)
// clang-format on
