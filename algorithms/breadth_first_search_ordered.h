...
inline void bfs(Ordered_Graph& graph, Ordered_Graph::Node_Pointer& start_node, std::function<void(Ordered_Graph::Node_Pointer&)> callback = nullptr) {
  ...
  while (!node_queue.empty()) {
    ...
    node_queue.pop();
    Ordered_Graph::Adjacency_Value* node_edges_ptr = graph.get_edge_nodes(node.key());
