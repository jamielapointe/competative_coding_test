...
inline void dfs(Unordered_Graph& graph, Unordered_Graph::Node_Pointer& start_node, std::function<void(Unordered_Graph::Node_Pointer&)> callback = nullptr) {
  start_node->is_visited = true;
  if (callback) {
    callback(start_node);
  }
  Unordered_Graph::Adjacency_Value* node_edges_ptr = graph.get_edge_nodes(start_node->key());
